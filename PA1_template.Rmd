...
The mean number of steps taken per day is `r st_mean`, the median is `r st_med`
...
The most active interval is `r result1[[3]]` with an average of `r result1[[2]]` steps.
...
`r NAs` rows have missing data, and `r NA_means` averaged intervals have missing data. So we can substitute the missing values in the original data with the average of their interval.
...
Now, the mean number of steps taken per day is `r st_mean2` (a difference of `r st_mean2 - st_mean`), the median is `r st_med2` (a difference of `r st_med2 - st_med`)
